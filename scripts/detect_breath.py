"""흡기/호기 onset + 무호흡 검출 → 화살표 오버레이 그래프.

읽기(breath.io_csv) → 필터(breath.filters) → 검출(breath.detectors) → 그리기(plotting)
순서로 조립만 한다. 실제 로직은 각 라이브러리에 있다.

실행:
    python scripts/detect_breath.py data/breath_...csv
"""
...
from breath.detectors import AmplitudeDetector, run_detector
...
DETECTOR_PARAMS = {}          # AmplitudeDetector 파라미터 오버라이드(예: {"k_delta":0.2})
# ============================================================


def run(path, save=None):
    if save is None:
        save = image_path_for(path, "_detect.png")

    times, _raws, mvs, phases = read_csv(path)
    fs = estimate_sample_rate(times)
    y_bp = bandpass_butter2(mvs, fs, HP, LP)

    det = AmplitudeDetector(**DETECTOR_PARAMS)
    ...
    print(f"무호흡   : {len(apnea_spans)}건")

    spans = target_spans(times, phases, config.INHALE_S, config.EXHALE_S)
    title = f"{os.path.basename(path)}   |  breath detection"
    if bpm:
        title += f"  ~{bpm:.1f} bpm"
